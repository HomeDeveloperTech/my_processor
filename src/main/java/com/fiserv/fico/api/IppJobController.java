...
@RestController
@RequestMapping("/api/ipp/jobs")
@RequiredArgsConstructor
public class IppJobController {

  private final IppJobService jobService;
  ...
  public ResponseEntity<Resource> download(@PathVariable("id") UUID id) throws IOException {
    ...
    switch (info.getStatus()) {
      case RUNNING:
        base = "PARCIAL_IPP-ALUGUEL_POSTADO_" + mmaa;
        break;
      case DONE:
        base = "FINAL_IPP_ALUGUEL_POSTADO_" + mmaa;
        break;
      default:
        return ResponseEntity.notFound().build();
    }
